...
<div class="w-full shadow-md rounded px-8 py-30">
    <ul class="py-12 px-80">
        <li class="rounded-full py-2 text-center border-2 border-gray-200">{{ $user->name }}</li>
        <br>
        <li class="rounded-full py-2 text-center border-2 border-gray-200">{{ $user->email }}</li>
    </ul>

<form action="{{ route('users.delete', $user->id) }}" method="POST" class="py-5  text-center">
    @method('DELETE')
    @csrf
